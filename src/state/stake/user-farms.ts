import { ChainId, JSBI, TokenAmount } from '@mutantswap/sdk'
import { MCOIN, AURORA } from '../../constants'
import { useComplexRewarderContract, useMasterChefV2ContractForVersion } from './hooks-sushi'
import { STAKING, StakingMCOIN, tokenAmount, ChefVersions } from './stake-constants'
...
export function useSingleFarm(version: number): StakingMCOIN {
  ...
  const pendingMC = useSingleCallResult(contract, 'pendingMC', v1args) //user related
  ...
  const result = useMemo(() => {
    ...
    const earnedRewardPool = JSBI.BigInt(pendingMC.result?.[0] ?? 0)
    ...
    const earnedAmount = new TokenAmount(MCOIN[ChainId.AURORA], JSBI.BigInt(earnedRewardPool))
  ...
  }, [chainId, userInfo, pendingMC, pendingComplexRewards, pairState, tokens, tokenA, tokenB, latestBlock])
