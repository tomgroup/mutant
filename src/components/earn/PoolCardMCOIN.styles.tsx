...
export const ButtonGoldGradient = styled(ButtonPrimary)<{ isStaking: boolean }>`
  background-image: linear-gradient(to bottom,  ${({ isStaking, theme }) => (isStaking ? theme.bg3 : theme.primary1)}, ${({ isStaking, theme }) => (isStaking ? theme.bg3 : theme.primary4)});
  padding: 8px;
  border-radius: 10px;
  max-width: 80px;
  ${({ isStaking, theme }) =>
    ...
    `
        ...
        max-width: fit-content;
        &:focus, &:hover, &:active {
            background-image: linear-gradient(to bottom, ${lighten(0.12, theme.primary1)}, ${lighten(0.12, theme.primary4)};
        }
  `}
